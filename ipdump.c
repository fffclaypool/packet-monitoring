...
/* パケットフィルタ用マクロ */
#define FILTER_ARP(_p) (filter.flg[IP_ADDR] ? ((*(int *)(_p)->arp_spa == filter.ip.s_addr || *(int *)(_p)->arp_tpa == filter.ip.s_addr) ? 1 : 0) \
                                            : 1)
#define FILTER_IP(_p) (filter.flg[IP_ADDR] ? (((_p)->ip_src.s_addr == filter.ip.s_addr || (_p)->ip_dst.s_addr == filter.ip.s_addr) ? 1 : 0) \
                                           : 1)
#define FILTER_TCP(_p) (filter.flg[PORT] ? (((_p)->th_sport == filter.port || (_p)->th_dport == filter.port) ? 1 : 0) \
                                         : 1)
#define FILTER_UDP(_p) (filter.flg[PORT] ? (((_p)->uh_sport == filter.port || (_p)->uh_dport == filter.port) ? 1 : 0) \
                                         : 1)
...
int main(int argc, char **argv)
{
    ...
    char ifname[CMAX] = ""; /* インターフェース名 */
    int opt[OPTNUM];        /* 表示オプションのフラフ */
...
void print_ip(struct ip *ip)
{
    ...
    printf("+-----+------+--------------------------------------+\n");
    printf("| Identifier:        %5u| FF:%3s| F0:        %5u|\n",
...
char *ip_ttoa(int flag)
{
    static int f[] = {'1', '1', '1', 'D', 'T', 'R', 'C', 'X'}; /* TOSフィールドを表す文字 */
...
void print_icmp(struct icmp *icmp)
{
    static char *type_name[] = {
        ...
        "Unknown" /* Type 19 */};  /* ICMPのタイプを表す文字列 */
...
int open_bpf(char *ifname, int *bufsize)
{
    ...
    struct ifreq ifr; /* インターフェース属性構造体 */
    int i;            /* ループ変数 */
    ...
    *bufsize = MAXSIZE;
    ...
    {
        snprintf(buf, CMAX, "ioctl(BIOCSBLEN, '%s')", ifname);
        perror(buf);
        return -1;
    }
...
void help(char *cmd)
{
    ...
    fprintf(stderr, "protocols: arp p icmp tcp udp other\n");
